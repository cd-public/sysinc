...
- This is an extended homework of more than week, due after break the midterm. I recommend:

| Week:    | Tasks:    |
| --------- | ---------- |
| 15 Oct | 2048 bit prime generation  |
| 22 Oct | Unsigned big extended GCD |
| 29 Oct | Finish Key Generation and File I/O |
...
This is the impossible assignment. Implement the entire assignment using your `4096_t`. At some point it will become clear that your `4096_t` is not performant enough to every compute a key. At that time, switch to using the GMP library and procede. However, you should in all cases ensure that it *could* be the case that could use your own code if you had infinite time or computing resources.
...
- It is reasonable to test 512 (`openssl` minimum) or 1024 (`ssh-keygen` minumum size)
...
- In lieu, I am providing a full reference implementation, with a few caveats:
  ...
  - I am trusting you to either look at, or not look at, my code, based on what works best for your learning.
    - It is provided as is, and likely has few bugs, but probably not too many.
