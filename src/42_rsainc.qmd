...
RUN curl https://raw.githubusercontent.com/cd-c89/sysinc/refs/heads/main/rsainc/tester.py -o tester.py
```
...
- This assignment was essentially as follows:
  - Create a `keygen.c` that compiles into a `keygen` executable which is functionally identical to `python3 keygen.py`
    - Key generation should create an `unsafe.bad` private key and an `unsafe.pub` public key.
  - Create a `rsainc.c` that compiles into a `rsainc` executable which is functionally identical to `python3 rsa_py.py`
...
- The tester:
  ...
  - Compiles `keygen.c` and `rsainc.c` with all flags:
```{.email}
